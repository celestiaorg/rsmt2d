...
func TestRepairExtendedDataSquare(t *testing.T) {
    ...
    result, err = RepairExtendedDataSquare(original.RowRoots(), original.ColumnRoots(), flattened)
    ...
    result, err = RepairExtendedDataSquare(original.RowRoots(), original.ColumnRoots(), corrupted.flattened())
    if err == nil {
    ...
    corrupted, err = original.deepCopy()
    corrupted.setCell(0, 0, []byte{66})
    result, err = RepairExtendedDataSquare(corrupted.RowRoots(), corrupted.ColumnRoots(), corrupted.flattened())
    ...
    result, err = RepairExtendedDataSquare(corrupted.RowRoots(), corrupted.ColumnRoots(), flattened)
