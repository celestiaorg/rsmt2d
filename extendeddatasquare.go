...
func ComputeExtendedDataSquare(data [][]byte, codec int) (*ExtendedDataSquare, error) {
    if value, ok := SupportedCodecs[codec]; ok {
        if len(data) > value {
...
func ImportExtendedDataSquare(data [][]byte, codec int) (*ExtendedDataSquare, error) {
    if value, ok := SupportedCodecs[codec]; ok {
        if len(data) > value * 4 {
            return nil, errors.New("number of chunks exceeds the maximum")
        }
